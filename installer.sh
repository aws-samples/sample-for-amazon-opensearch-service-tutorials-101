...
project_name="opensearch-tutorial-builder-$infra_env-$timestamp"

# Check if the service role exists, if not create it
if ! aws iam get-role --role-name codebuild-opensearch-tutorial-service-role --query 'Role.RoleName' --output text &>/dev/null; then
  echo "Creating CodeBuild service role..."
  
  # Create the role
  aws iam create-role \
    --role-name codebuild-opensearch-tutorial-service-role \
...
input_bucket="codebuild-$infra_env-$deployment_region-$account_id-input-bucket"
...
zip -r sample-for-amazon-opensearch-tutorials-101.zip . -x "*.git*" "*.zip"

# Upload to S3
echo "Uploading project files to S3..."
aws s3 cp sample-for-amazon-opensearch-tutorials-101.zip s3://$input_bucket/
...
response=$(aws codebuild create-project \
  --name $project_name \
  --source type=S3,location=codebuild-$infra_env-$deployment_region-$account_id-input-bucket/sample-for-amazon-opensearch-tutorials-101.zip \
  --artifacts type=NO_ARTIFACTS \
  --environment type=LINUX_CONTAINER,image=aws/codebuild/amazonlinux2-x86_64-standard:5.0,computeType=BUILD_GENERAL1_MEDIUM,privilegedMode=true,environmentVariables="[{name=infra_env,value=$infra_env,type=PLAINTEXT}]" \
  --service-role codebuild-opensearch-tutorial-service-role \
...
while true; do
  ...
  if [ "$status" == "SUCCEEDED" ] || [ "$status" == "FAILED" ] || [ "$status" == "STOPPED" ]; then
    ...
    echo "Build is still in progress (Usually takes 45-50 minutes to complete)... sleeping for 60 seconds"
...
# Clean up
rm -f sample-for-amazon-opensearch-tutorials-101.zip
rm -f buildspec.yml
